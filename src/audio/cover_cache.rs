...
use lofty::{prelude::*, picture::PictureType, tag::{ItemValue, Tag}};
use log::debug;
...
impl CoverCache {
        ...
        if let Some(picture) = tag.get_picture_type(PictureType::CoverFront) {
            ...
            for picture in tag.pictures() {
                let cover_art = match picture.pic_type() {
                    PictureType::Other => Some(glib::Bytes::from(picture.data())),
                    PictureType::BandLogo => Some(glib::Bytes::from(picture.data())),
                    _ => None,
                };
    ...
    pub fn cover_art(&mut self, path: &Path, tag: &Tag) -> Option<(CoverArt, String)> {
        ...
        fn get_text_value(value: &ItemValue) -> Option<String> {
            match value {
        ...
        for item in tag.items() {
            match item.key() {
                ItemKey::AlbumTitle => album = get_text_value(item.value()),
                ItemKey::AlbumArtist => album_artist = get_text_value(item.value()),
                ItemKey::TrackArtist => track_artist = get_text_value(item.value()),
                _ => (),
            };
        }
